  ...
  return (
    <>
      <motion.button
        ...
        style={{
          background:
            'linear-gradient(150deg, #F5B73C 0%, #F47A1F 45%, #D63B36 75%, #C13584 100%)',
        }}
        whileHover={{ scale: 1.02, rotate: -1 }}
        aria-label={`Reproducir reel de Instagram: ${title}`}
      >
        <div
        ...
        <svg
          className="h-10 w-10 text-blanco/90 drop-shadow"
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden
        >
        ...
        <div className="relative z-10 mt-4 flex h-16 w-16 items-center justify-center rounded-full border-4 border-negro bg-blanco shadow-[3px_3px_0_var(--negro)] transition-transform group-hover:scale-110">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-negro/80 to-transparent p-4 text-left">
          ...
          {description && (
            <p className="font-body text-xs text-blanco/85">{description}</p>
          )}
        </div>
      </motion.button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            >
              <button
                className="absolute -top-12 right-0 cursor-pointer text-3xl text-blanco/80 hover:text-sol"
                onClick={() => setOpen(false)}
                aria-label="Cerrar vídeo"
              >
                ✕
              </button>
              <div className="max-h-[80vh] overflow-y-auto rounded-2xl border-4 border-sol bg-blanco">
                <blockquote
              ...
              <p className="mt-3 text-center font-display text-sm italic text-blanco/80">
                {title}
